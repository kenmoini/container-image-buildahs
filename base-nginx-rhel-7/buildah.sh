...
NAME=base-nginx-rhel-7
NGINX_VERSION=1.14.2
NGINX_SHORT_VER=114

container=$(buildah from registry.gitlab.fiercesw.network/kemo-org/container-image-repo/base-rhel-7)
...
buildah run $container yum install -y yum-utils 
buildah run $container yum-config-manager --enable rhel-7-server-optional-rpms
buildah run $container yum-config-manager --enable rhel-7-server-extras-rpms
buildah run $container yum install -y curl wget gzip tar zip unzip gettext git tree perl perl-devel perl-ExtUtils-Embed libxslt libxslt-devel libxml2 libxml2-devel gd gd-devel GeoIP GeoIP-devel gcc-c++ libatomic_ops libatomic_ops-devel python-setuptools
buildah run $container yum groupinstall -y 'Development Tools'
buildah run $container yum clean all
buildah run $container easy_install pip
buildah run $container pip install supervisor
...
buildah commit --format docker $container base-nginx-rhel-7:latest
